...
files <- list.files(path="P:/Downloads/articles", pattern="*.html", full.names=TRUE, recursive=FALSE)

for(i in 1:length(files)){
  title<-files[i]%>%read_html()%>%html_nodes(".article-title-main")%>%html_text()
  ...
  DOI_b<-files[i]%>%read_html()%>%html_nodes(".ww-citation-primary a")%>%html_text()
  ...
  authors<-files[i]%>%read_html()%>%html_nodes(".linked-name")%>%html_text()
  ...
  publication_date<-files[i]%>%read_html()%>%html_nodes(".citation-date")%>%html_text()
  ...
  abstract<-files[i]%>%read_html()%>%html_nodes(".abstract p")%>%html_text()
  ...
  key_words<-files[i]%>%read_html()%>%html_nodes(".kwd-main")%>%html_text()
  ...
  full_text<-files[i]%>%read_html()%>%html_nodes("#ContentColumn")%>%html_text()
  ...
  write.table(required_output, "DNAresearch.txt", row.names = FALSE, col.names=FALSE, sep = "\t",append = T)
}